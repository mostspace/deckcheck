    ...
    <form id="task-form"
        ...
        <div>
            ...
            <input id="description" name="description" value="{{ old('description') }}"
                class="w-full rounded-lg border border-[#cfd4dc] bg-white px-3.5 py-2.5 text-[#0f1728] focus:ring-[#6840c6]"
            ...
            <div class="space-y-3">
                @foreach ($options as $label => $opt)
                    <div class="flex items-start gap-3">
                        <input type="radio" id="{{ $opt['id'] }}" name="applicable_to" value="{{ $opt['value'] }}"
                            class="mt-1 h-4 w-4 text-[#6840c6] focus:ring-[#6840c6]"
                            {{ $applicableTo === $opt['value'] ? 'checked' : '' }}>
                        <div class="flex-1">
                            <label for="{{ $opt['id'] }}" class="cursor-pointer text-sm font-medium text-[#0f1728]">
                                {{ $label }}
                            </label>
        ...
        <div id="specific-equipment-section"
            ...
            <div class="space-y-2">
                @foreach ($interval->category->equipment as $eq)
                    <label class="flex items-center gap-2 rounded-lg border border-[#e4e7ec] bg-white px-4 py-3 shadow-sm">
