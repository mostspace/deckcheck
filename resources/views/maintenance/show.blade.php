...
@section('content')
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <thead class="bg-[#f8f9fb]">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]">Interval
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]">
                            Requirement</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]">Tasks
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]">Affected
                            Equipment</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]">
                            Facilitator</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]">Actions
                        </th>
                    </tr>
                </thead>
                ...
                <tbody class="divide-y divide-[#e4e7ec]">
                    ...
                    @if ($category->intervals->isNotEmpty())
                        ...
                        @foreach ($category->intervals as $interval)
                            <tr class="hover:bg-[#f9f5ff]">
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-[#0f1728]">{{ $interval->description }}
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-[#0f1728]">{{ $interval->tasks_count }}
                                </td>
                    ...
                    @else
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-sm text-[#667084]">No intervals defined for
                                category yet.</td>
                        </tr>
                    @endif
    ...
    <section id="equipment-manifest-section" class="rounded-lg border border-[#e4e7ec] bg-white">
            ...
            <table class="w-full">
                ...
                <thead class="bg-[#f8f9fb]">
                    <tr class="bg-[#f8f9fb]">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]">Serial
                            Number</th>
                        <th class="px-6 py-3 text-left font-medium">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]">Location
                        </th>
                        <th class="px-6 py-3 text-left font-medium">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#7e56d8]"></th>
                    </tr>
                </thead>
                        ...
                        @foreach ($equipList as $eq)
                            <tr class="hover:bg-[#f9f5ff]" data-id="{{ strtolower($eq->internal_id) }}"
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-[#0f1728]">
                                    {{ $eq->internal_id ?? '-' }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-[#0f1728]">{{ $eq->name ?? '—' }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-[#0f1728]">
                                    {{ $eq->serial_number ?? '—' }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-[#0f1728]">
                                    {{ $eq->deck->name ?? '—' }}</td>
        ...
        <div class="flex items-center justify-between border-t border-[#e4e7ec] px-6 py-4">
            ...
            <!--<div class="inline-flex h-10 items-start justify-start shadow-[0px_1px_2px_0px_rgba(16,24,40,0.05)]">
                                <div
                                    class="flex items-center justify-center gap-2 overflow-hidden rounded-bl-lg rounded-tl-lg border border-[#cfd4dc] bg-white px-4 py-2.5">
                                    <i class="fa-solid fa-chevron-left text-sm text-[#1d2838]"></i>
                                    <div class="text-sm leading-tight text-[#1d2838]">Previous</div>
                                </div>
                                <div class="inline-flex w-10 flex-col items-center justify-center bg-[#f9f5ff]">
                                    <div class="h-px self-stretch bg-[#cfd4dc]"></div>
                                    <div class="inline-flex items-center justify-center gap-2.5 px-2 py-[9px]">
                                        <div class="text-sm leading-tight text-[#6840c6]">1</div>
                                    </div>
                                    <div class="h-px self-stretch bg-[#cfd4dc]"></div>
                                </div>
                                <div class="inline-flex w-10 flex-col items-center justify-center bg-white">
                                    <div class="h-px self-stretch bg-[#cfd4dc]"></div>
                                    <div class="inline-flex items-center justify-center gap-2.5 px-2 py-[9px]">
                                        <div class="text-sm leading-tight text-[#344053]">2</div>
                                    </div>
                                    <div class="h-px self-stretch bg-[#cfd4dc]"></div>
                                </div>
                                <div class="inline-flex w-10 flex-col items-center justify-center bg-white">
                                    <div class="h-px self-stretch bg-[#cfd4dc]"></div>
                                    <div class="inline-flex items-center justify-center gap-2.5 px-2 py-[9px]">
                                        <div class="text-sm leading-tight text-[#344053]">3</div>
                                    </div>
                                    <div class="h-px self-stretch bg-[#cfd4dc]"></div>
                                </div>
                                <div
                                    class="flex items-center justify-center gap-2 overflow-hidden rounded-br-lg rounded-tr-lg border border-[#cfd4dc] bg-white px-4 py-2.5">
                                    <div class="text-sm leading-tight text-[#1d2838]">Next</div>
