...
@section('form')
    <form action="{{ route('maintenance.update', $category) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <div id="icon-picker" class="grid w-1/2 grid-cols-8 gap-4">
                @foreach ($icons as $icon)
                    <div class="{{ old('icon', $category->icon) === $icon ? 'bg-violet-50 border-white' : 'border-white' }} relative flex h-8 w-8 cursor-pointer items-center justify-center rounded-md border bg-[white] transition-all"
                        data-icon="{{ $icon }}" onclick="selectIcon(this)">
                        <i
                            class="fa-solid {{ $icon }} {{ old('icon', $category->icon) === $icon ? 'text-[#6840c6]' : 'text-[#6840c6]' }} text-[16px]"></i>

                        @if (old('icon', $category->icon) === $icon)
                            <div class="checkmark absolute -right-1.5 -top-1.5 rounded-full text-[10px] text-green-600">
