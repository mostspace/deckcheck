...
<div class="file-upload-component {{ $class }}" x-data="fileUpload()">
        ...
        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors"
            ...
            <div x-show="!dragover" class="space-y-2">
                ...
                <div class="text-sm text-gray-600">
                    <label for="file-upload-{{ $role ?? 'default' }}" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                        <span>Upload files</span>
                        <input 
                            id="file-upload-{{ $role ?? 'default' }}"
                            name="{{ $inputName }}"
                            type="file"
                            class="sr-only"
                            {{ $multiple ? 'multiple' : '' }}
        ...
        <template x-for="(upload, index) in uploads" :key="index">
            <div class="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                <div class="flex-shrink-0">
                    <div x-show="upload.status === 'uploading'" class="animate-spin rounded-full h-5 w-5 border-b-2 border-indigo-600"></div>
                    <div x-show="upload.status === 'success'" class="rounded-full h-5 w-5 bg-green-500 flex items-center justify-center">
                        <svg class="h-3 w-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                        </svg>
                    </div>
                    <div x-show="upload.status === 'error'" class="rounded-full h-5 w-5 bg-red-500 flex items-center justify-center">
                        <svg class="h-3 w-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-900 truncate" x-text="upload.file.name"></p>
                    <p class="text-sm text-gray-500" x-text="upload.status === 'uploading' ? 'Uploading...' : upload.status === 'success' ? 'Upload complete' : upload.error"></p>
                </div>
    ...
    <div x-show="uploads.length > 0" class="mt-4">
        <button 
            @click="uploadFiles()"
            :disabled="!canUpload"
            class="w-full bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
    ...
    <!-- Success Message -->
    <div x-show="uploadSuccess" class="mt-4 p-4 bg-green-50 border border-green-200 rounded-md">
        <div class="flex">
            <div class="flex-shrink-0">
    ...
    <!-- Error Message -->
    <div x-show="uploadError" class="mt-4 p-4 bg-red-50 border border-red-200 rounded-md">
        <div class="flex">
            <div class="flex-shrink-0">
