@props([
    'tabs' => [['id' => 'overview', 'label' => 'Overview', 'icon' => 'tab-overview.svg', 'active' => true]],
    'breadcrumbs' => [],
    'actions' => [],
    'showSubHeader' => true,
    'showTopHeader' => true
...
<header class="relative z-20 flex flex-col">
    @if ($showTopHeader)
        <div id="top-header"
            ...
            <!-- Main Tab -->
            <div class="no-scrollbar -mb-px flex max-w-full items-center gap-0.5 overflow-x-auto sm:gap-1"
                role="tablist" aria-label="Primary tabs">
                @foreach ($tabs as $index => $tab)
                    <button id="tab-{{ $tab['id'] }}" role="tab"
                        aria-selected="{{ $tab['active'] ? 'true' : 'false' }}"
                        aria-controls="panel-{{ $tab['id'] }}" tabindex="{{ $tab['active'] ? '0' : '-1' }}"
                        onclick="switchTab('{{ $tab['id'] }}')"
                        @if ($tab['active']) data-accent="true" 
                    ...
                    @else
                        class="px-2 sm:px-3 py-1.5 text-xs sm:text-sm whitespace-nowrap flex items-center gap-1 sm:gap-2 border hover:bg-white text-slate-700 rounded-t-md flex-shrink-0" @endif>
                        @if (isset($tab['icon']) && !empty($tab['icon']))
                            ...
                            <img src="{{ asset('assets/media/icons/' . $iconName) }}"
                                ...
                                class="tab-icon {{ $tab['active'] || $tab['id'] === 'workflow' ? 'text-slate-900' : 'text-slate-500' }} h-4 w-4"
                                alt="{{ $tab['label'] }}" />
                        @endif
        ...
        <div class="flex items-center rounded-lg bg-white" id="breadcrumb-container">
            ...
            @else
                ...
                <span
                    class="shadow-soft z-10 inline-flex items-center gap-1 rounded-lg border border-primary-500 bg-accent-200/40 px-2 py-1.5 text-xs text-slate-900 sm:gap-2 sm:px-3"
                    id="page-breadcrumb">
                    <img src="{{ $pageIcon ?? asset('assets/media/icons/sidebar-solid-wrench-scredriver.svg') }}"
                        alt="{{ $pageName ?? 'Page' }}" class="h-3 w-3" />
                    <span>{{ $pageName ?? 'Page' }}</span>
                </span>
                <span
                    class="-ml-4 inline-flex items-center rounded-lg border border-l-0 border-slate-200 px-2 py-1.5 pl-6 text-xs text-slate-500 sm:-ml-5 sm:px-3 sm:pl-7"
                    id="current-tab-breadcrumb">{{ $activeTab ?? 'Tab' }}</span>
            @endif
        </div>
        ...
        <div class="hidden items-center gap-2 sm:flex lg:gap-4">
            @if (count($actions) > 0)
                @foreach ($actions as $action)
                    <button class="icon-hover-btn" title="{{ $action['title'] }}"
                        aria-label="{{ $action['aria_label'] ?? $action['title'] }}">
                        <img src="{{ $action['icon'] }}" class="icon-normal h-6 w-6"
                            alt="{{ $action['title'] }}" />
                        @if (isset($action['icon_solid']))
                            <img src="{{ $action['icon_solid'] }}" class="icon-solid hidden h-6 w-6"
                                alt="{{ $action['title'] }}" />
                        @endif
                    </button>
                @endforeach
            @else
                ...
                <button class="icon-hover-btn" title="Share" aria-label="Share">
                    <img src="{{ asset('assets/media/icons/plus-circle.svg') }}" class="icon-normal h-6 w-6"
                        alt="Share" />
                    <img src="{{ asset('assets/media/icons/plus-circle-solid.svg') }}"
                        class="icon-solid hidden h-6 w-6" alt="Share" />
                </button>
                <button class="icon-hover-btn" title="Settings" aria-label="Settings">
                    <img src="{{ asset('assets/media/icons/chat-bubble.svg') }}" class="icon-normal h-6 w-6"
                        alt="Settings" />
                    <img src="{{ asset('assets/media/icons/chat-bubble-solid.svg') }}"
                        class="icon-solid hidden h-6 w-6" alt="Settings" />
                </button>
...
<script>
    ...
    function switchTab(tabId) {
        ...
        tabButtons.forEach(tab => {
            ...
            tab.classList.remove('px-2', 'sm:px-3', 'py-1.5', 'rounded-t-md', 'rounded-b-none', 'text-xs',
                'sm:text-sm', 'bg-white', 'bg-primary-500', 'bg-opacity-50', 'bg-opacity-100',
                'text-slate-900', 'text-slate-700', 'border', 'border-[#E4E4E4]', 'border-primary-500',
                'border-b-0', 'border-b-transparent', 'whitespace-nowrap', 'flex', 'items-center', 'gap-1',
                'sm:gap-2', 'flex-shrink-0', 'hover:bg-white', 'hover:bg-primary-500',
                'hover:bg-opacity-100');

            // Check if this is the workflow tab
            const isWorkflowTab = tab.id === 'tab-workflow';
            if (isWorkflowTab) {
                tab.classList.add('px-2', 'sm:px-3', 'py-1.5', 'text-xs', 'sm:text-sm', 'whitespace-nowrap',
                    'flex', 'items-center', 'gap-1', 'sm:gap-2', 'border', 'bg-primary-500',
                    'bg-opacity-50', 'hover:bg-primary-500', 'hover:bg-opacity-100', 'text-slate-900',
                    'rounded-t-md', 'flex-shrink-0');
            } else {
                tab.classList.add('px-2', 'sm:px-3', 'py-1.5', 'text-xs', 'sm:text-sm', 'whitespace-nowrap',
                    'flex', 'items-center', 'gap-1', 'sm:gap-2', 'border', 'hover:bg-white',
                    'text-slate-700', 'rounded-t-md', 'flex-shrink-0');
            }
