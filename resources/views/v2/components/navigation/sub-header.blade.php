    ...
    <div class="flex items-center rounded-md bg-white">
            ...
            @foreach($breadcrumbs as $index => $crumb)
                @if($index === 0)
                    <span class="inline-flex items-center gap-1 sm:gap-2 text-xs px-2 sm:px-3 py-1.5 rounded-md border border-primary bg-accent-200/40 text-slate-900 z-10 shadow-soft">
                        ...
                        @endif
                        <span class="hidden xs:inline">{{ $crumb['label'] }}</span>
                        <span class="xs:hidden">{{ $crumb['short'] ?? $crumb['label'] }}</span>
                    </span>
                @else
        ...
        @else
            ...
            <span class="inline-flex items-center gap-1 sm:gap-2 text-xs px-2 sm:px-3 py-1.5 rounded-md border border-primary bg-accent-200/40 text-slate-900 z-10 shadow-soft">
                ...
                <span class="hidden xs:inline">Maintenance</span>
                <span class="xs:hidden">Maint.</span>
            </span>
            <span class="inline-flex items-center text-xs px-2 sm:px-3 pl-6 sm:pl-7 py-1.5 -ml-4 sm:-ml-5 text-slate-500 rounded-md border border-slate-200">Index</span>
        @endif
    </div>
