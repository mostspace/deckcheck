...
<body class="text-base-content h-dvh bg-white">

    @php
        $user = auth()->user();
    @endphp

    <div id="app-layout" class="grid h-full w-full md:grid-cols-[56px_1fr]">
        ...
        <main class="relative flex h-full flex-col overflow-hidden">
            <section class="relative min-h-0 flex-1">
                <div id="main-content" class="h-full overflow-y-auto rounded-md bg-white">
    ...
    <div id="ee-modal" class="fixed inset-0 z-[9999] hidden">
        ...
        <canvas id="ee-canvas" class="absolute inset-0 h-full w-full"></canvas>
        <div class="absolute left-0 right-0 top-0 flex items-center justify-between p-3 text-xs text-white">
            <div class="opacity-80">Press <kbd class="rounded bg-white/20 px-1 py-0.5">Esc</kbd> or <kbd
                    class="rounded bg-white/20 px-1 py-0.5">Space</kbd> to exit</div>
    ...
    <script>
        (function() {
            ...
            const SEQUENCE = ['ArrowUp', 'ArrowUp', 'ArrowDown', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'ArrowLeft',
                'ArrowRight', 'b', 'a'
            ];
            ...
            function rotateMatrix(m) {
                ...
                for (let r = 0; r < N; r++)
                    for (let c = 0; c < M; c++) res[c][N - 1 - r] = m[r][c];
                return res;
            }

            function newBag() {
                return Object.keys(shapes).sort(() => Math.random() - 0.5);
            }
            ...
            function drawBackground(t) {
                ...
                // small sailboat icon
                const boatY = originY - 20;
                const boatX = originX + boardPixelW - 60;
