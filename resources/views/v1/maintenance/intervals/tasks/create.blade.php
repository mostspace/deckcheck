...
@php
    $title = 'New Task';
    $subtitle =
        "Define a new task for <span class=\"font-semibold uppercase " .
...
>
    ...
    <div>
        ...
        <input
            ...
            class="w-full px-3.5 py-2.5 bg-white rounded-lg border border-[#cfd4dc]
                   text-[#0f1728] focus:ring-[#6840c6]"
            ...
            @foreach($options as $label => $opt)
                <div class="flex items-start gap-3">
                    <input
                        ...
                        class="mt-1 w-4 h-4 text-[#6840c6] focus:ring-[#6840c6]"
          ...
          <div class="flex items-center gap-3 condition-row">
            <select class="condition-key w-1/3 border border-[#cfd4dc] rounded-lg px-3 py-2">
              ...
              @foreach($staticConditions as $key => $vals)
                <option value="{{ $key }}">{{ $keyLabels[$key] ?? ucfirst(str_replace('_id','',$key)) }}</option>
              @endforeach
    ...
    <div class="flex justify-end space-x-4">
      <a href="{{ route('maintenance.intervals.show',['category'=>$interval->category,'interval'=>$interval]) }}"
         class="px-4 py-2.5 bg-white text-[#344053] border border-[#cfd4dc]
                rounded-lg text-sm font-medium hover:bg-[#f9fafb]">
        Cancel
      </a>
      <button type="submit"
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  radios.forEach(r=>{ if(r.checked) toggle(r.value); });
});
  ...
  function bindRow(row) {
    ...
    keySel.addEventListener('change', () => {
      ...
      for (const v in opts) {
        ...
        op.value = v; op.textContent = opts[v];
